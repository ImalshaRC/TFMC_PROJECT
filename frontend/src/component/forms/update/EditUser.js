...
const EditUser = () => {
    ...
    const formValidation = () =>{
      ...
      if(!fname.match(/^([a-zA-Z]{2,15})$/)){
        toast.error("Please insert invalid first name");
      }
      else if(!lname.match(/^([a-zA-Z]{2,15})$/)){
        toast.error("Please insert invalid last name");
      }
      else if(dob === ""){
        toast.error("Please insert invalid date of birth");
      }
      else if(!nic.match(/^([0-9]{9,9})([A-Za-z]{1,1})$/) && !nic.match(/^([0-9]{10,10})$/)){
        toast.error("Please insert invalid NIC number");
      }
      ...
      else if(!String(salary).match(/^([0-9]{2,6})$/)){
        toast.error("Please insert correct salary amount");
      }
      else if(!String(tp).match(/^([0-9]{9,10})$/)){
        toast.error("Please insert correct mobile number");
      }
    ...
    <form onSubmit={e => onSubmit(e)}><br/>
      ...
      <table class="signup-table">
        <tr>
          ...
          <td>
            Date Of Birth<br/>
            <input type="text" name="dob" class="insert" value={dob} onChange={ e => onInputChange(e)} required />
          </td>
        </tr>
        <tr>
          <td>
      ...
      <Link to={`/employeeManagement/`}><button class="button">back</button></Link>
      </center>
