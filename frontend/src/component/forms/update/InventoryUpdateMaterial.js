...
class InventoryUpdateMaterial extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount(){
    axios.get('http://localhost:5000/material/get/'+this.props.match.params.id).then(res =>{
        this.setState({
          sku:res.data.material.SKU,
          name:res.data.material.Name,
          available_units:res.data.material.Available_Units,
          unit_price:res.data.material.Unit_Price,
          net_weight:res.data.material.Net_Weight
        });
  ...
  onSubmit(e) {
    ...
    axios.put('http://localhost:5000/material/edit/'+this.props.match.params.id, material).then(()=>{
...
export default InventoryUpdateMaterial;